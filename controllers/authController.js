...
const userRegister = async (req, res) => {
  try {
    ...
    const verficationTokenExpires = Date.now() + 1 * 60 * 60 * 1000;
    const user = await User.create({
      ...
      verificationTokenExpires: verficationTokenExpires,
    });
    const verificationUrl = `${API_URL}/api/v1/auth/verify/${verificationToken}`;
    try {
      ...
      if (verifyEmail) {
        ...
        return res.status(500).json({
          message: "Failed sending verification to email",
        });
      }
...
const deliverymanRegister = async (req, res) => {
  try {
    const { email, password, first_name, last_name, phone_number, shopname } =
      req.body;
